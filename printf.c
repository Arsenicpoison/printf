...
/**
 * cleanup - peforms cleanup operations .
 * @args:  va_list of arguments provided.
 ...
 * @format: Character string to print that should contain directives.
...
int run_printf(const char *format, va_list args, buffer_t *output)
{
	...
	for (count = 0; *(format + count); count++)
	{
		...
		if (*(format + count) == '%')
		{
			...
			prec = handle_precision(args, format + count + tmp + 1,
					&tmp);
 ...
 * @format: Character string to print - should contain directives.
